...
(let ((lambd-rpls '(("\u225C" ":=")
  ...
  (labels ((elam->smlam (s)
        ...
        (defun exec-lamb-command (command)
        ...
        (defun exec-lamb-rename ()
          (interactive)
          (exec-lamb-command "#rename "))

        (defun lamb-selection ()
          (interactive)
          (let ((start (buffer-substring-no-properties (line-beginning-position) (region-beginning)))
                (sel (buffer-substring-no-properties (region-beginning) (region-end)))
                (end (buffer-substring-no-properties (region-end) (line-end-position)))
                (p nil))
            (goto-char (line-end-position))
            (insert "\n")
            (insert start)
